...
from .handlers import Handler, InitHandler, DynamicHandler, make_embedded_args_template
...
class _BaseTestLibrary(object):
    ...
    def _create_handlers(self, libcode):
        for name in self._get_handler_names(libcode):
            ...
            if method:
                ...
                if handler:
                    self.handlers.add(handler, '$' in handler.name)
    ...
    def _create_handler(self, handler_name, handler_method):
        ...
        if '$' in handler.name:
            ...
            if embedded:
                handler = make_embedded_args_template(self, handler_name, handler_method, embedded, handler)
        return handler
